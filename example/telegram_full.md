...
#### Chat
    ...
    Chat(id: Integer, type: String, title: String, username: String, first_name: String, last_name: String, is_forum: Boolean, is_direct_messages: Boolean)
...
| is_direct_messages | Boolean | false | <em>Optional</em>. <em>True</em>, if the chat is the direct messages chat of a channel |
...
| parent_chat | Chat | false | <em>Optional</em>. Information about the corresponding channel chat; for direct messages chats only |
...
| reply_to_checklist_task_id | Integer | false | <em>Optional</em>. Identifier of the specific checklist task that is being replied to |
...
| is_paid_post | Boolean | false | <em>Optional</em>. <em>True</em>, if the message is a paid post. Note that such posts must not be deleted for 24 hours to receive the payment and can't be edited. |
...
| suggested_post_info | SuggestedPostInfo | false | <em>Optional</em>. Information about suggested post parameters if the message is a suggested post in a channel direct messages chat. If the message is an approved or declined suggested post, then it can't be edited. |
...
| pinned_message | MaybeInaccessibleMessage | false | <em>Optional</em>. Specified message was pinned. Note that the <a href="#message">Message</a> object in this field will not contain further <em>reply_to_message</em> fields even if it itself is a reply. |
...
| suggested_post_approved | SuggestedPostApproved | false | <em>Optional</em>. Service message: a suggested post was approved |
| suggested_post_approval_failed | SuggestedPostApprovalFailed | false | <em>Optional</em>. Service message: approval of a suggested post has failed |
| suggested_post_declined | SuggestedPostDeclined | false | <em>Optional</em>. Service message: a suggested post was declined |
| suggested_post_paid | SuggestedPostPaid | false | <em>Optional</em>. Service message: payment for a suggested post was received |
| suggested_post_refunded | SuggestedPostRefunded | false | <em>Optional</em>. Service message: payment for a suggested post was refunded |
...
#### ReplyParameters
    ...
    ReplyParameters(message_id: Integer, chat_id: IntegerOrString, allow_sending_without_reply: Boolean, quote: String, quote_parse_mode: String, quote_entities: List<MessageEntity>, quote_position: Integer, checklist_task_id: Integer)
...
| checklist_task_id | Integer | false | <em>Optional</em>. Identifier of the specific checklist task to be replied to |
...
| parse_mode | ParseMode | false | <em>Optional</em>. Mode for parsing entities in the text. See <a href="https://core.telegram.org/bots/api#formatting-options">formatting options</a> for more details. |
...
| parse_mode | ParseMode | false | <em>Optional</em>. Mode for parsing entities in the title. See <a href="#formatting-options">formatting options</a> for more details. |
...
| name | type | required | description |
|---|---|---|---|
| checklist_message | Message | false | <em>Optional</em>. Message containing the checklist whose tasks were marked as done or not done. Note that the <a href="#message">Message</a> object in this field will not contain the <em>reply_to_message</em> field even if it itself is a reply. |
...
| name | type | required | description |
|---|---|---|---|
| checklist_message | Message | false | <em>Optional</em>. Message containing the checklist to which the tasks were added. Note that the <a href="#message">Message</a> object in this field will not contain the <em>reply_to_message</em> field even if it itself is a reply. |
...
#### SuggestedPostApproved
    ...
    SuggestedPostApproved(suggested_post_message: Message, price: SuggestedPostPrice, send_date: Integer)

<p>Describes a service message about the approval of a suggested post.</p>

| name | type | required | description |
|---|---|---|---|
| suggested_post_message | Message | false | <em>Optional</em>. Message containing the suggested post. Note that the <a href="#message">Message</a> object in this field will not contain the <em>reply_to_message</em> field even if it itself is a reply. |
| price | SuggestedPostPrice | false | <em>Optional</em>. Amount paid for the post |
| send_date | Integer | true | Date when the post will be published |

#### SuggestedPostApprovalFailed

    SuggestedPostApprovalFailed(suggested_post_message: Message, price: SuggestedPostPrice)
...
| name | type | required | description |
|---|---|---|---|
| suggested_post_message | Message | false | <em>Optional</em>. Message containing the suggested post whose approval has failed. Note that the <a href="#message">Message</a> object in this field will not contain the <em>reply_to_message</em> field even if it itself is a reply. |
| price | SuggestedPostPrice | true | Expected price of the post |

#### SuggestedPostDeclined

    SuggestedPostDeclined(suggested_post_message: Message, comment: String)
...
|---|---|---|---|
| suggested_post_message | Message | false | <em>Optional</em>. Message containing the suggested post. Note that the <a href="#message">Message</a> object in this field will not contain the <em>reply_to_message</em> field even if it itself is a reply. |
| comment | String | false | <em>Optional</em>. Comment with which the post was declined |

#### SuggestedPostPaid

    SuggestedPostPaid(suggested_post_message: Message, currency: String, amount: Integer, star_amount: StarAmount)
...
|---|---|---|---|
| suggested_post_message | Message | false | <em>Optional</em>. Message containing the suggested post. Note that the <a href="#message">Message</a> object in this field will not contain the <em>reply_to_message</em> field even if it itself is a reply. |
| currency | String | true | Currency in which the payment was made. Currently, one of “XTR” for Telegram Stars or “TON” for toncoins |
| amount | Integer | false | <em>Optional</em>. The amount of the currency that was received by the channel in nanotoncoins; for payments in toncoins only |
| star_amount | StarAmount | false | <em>Optional</em>. The amount of Telegram Stars that was received by the channel; for payments in Telegram Stars only |

#### SuggestedPostRefunded

    SuggestedPostRefunded(suggested_post_message: Message, reason: String)
...
|---|---|---|---|
| suggested_post_message | Message | false | <em>Optional</em>. Message containing the suggested post. Note that the <a href="#message">Message</a> object in this field will not contain the <em>reply_to_message</em> field even if it itself is a reply. |
| reason | String | true | Reason for the refund. Currently, one of “post_deleted” if the post was deleted within 24 hours of being posted or removed from scheduled messages without being posted, or “payment_refunded” if the payer refunded their payment. |
...
#### SuggestedPostPrice
    ...
    SuggestedPostPrice(currency: String, amount: Integer)

<p>Describes the price of a suggested post.</p>

| name | type | required | description |
|---|---|---|---|
| currency | String | true | Currency in which the post will be paid. Currently, must be one of “XTR” for Telegram Stars or “TON” for toncoins |
| amount | Integer | true | The amount of the currency that will be paid for the post in the <em>smallest units</em> of the currency, i.e. Telegram Stars or nanotoncoins. Currently, price in Telegram Stars must be between 5 and 100000, and price in nanotoncoins must be between 10000000 and 10000000000000. |

#### SuggestedPostInfo

    SuggestedPostInfo(state: String, price: SuggestedPostPrice, send_date: Integer)

<p>Contains information about a suggested post.</p>

| name | type | required | description |
|---|---|---|---|
| state | String | true | State of the suggested post. Currently, it can be one of “pending”, “approved”, “declined”. |
| price | SuggestedPostPrice | false | <em>Optional</em>. Proposed price of the post. If the field is omitted, then the post is unpaid. |
| send_date | Integer | false | <em>Optional</em>. Proposed send date of the post. If the field is omitted, then the post can be published at any time within 30 days at the sole discretion of the user or administrator who approves it. |

#### SuggestedPostParameters

    SuggestedPostParameters(price: SuggestedPostPrice, send_date: Integer)

<p>Contains parameters of a post that is being suggested by the bot.</p>

| name | type | required | description |
|---|---|---|---|
| price | SuggestedPostPrice | false | <em>Optional</em>. Proposed price for the post. If the field is omitted, then the post is unpaid. |
| send_date | Integer | false | <em>Optional</em>. Proposed send date of the post. If specified, then the date must be between 300 second and 2678400 seconds (30 days) in the future. If the field is omitted, then the post can be published at any time within 30 days at the sole discretion of the user who approves it. |

#### DirectMessagesTopic

    DirectMessagesTopic(topic_id: Integer, user: User)

<p>Describes a topic of a direct messages chat.</p>

| name | type | required | description |
|---|---|---|---|
| topic_id | Integer | true | Unique identifier of the topic. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a 64-bit integer or double-precision float type are safe for storing this identifier. |
| user | User | false | <em>Optional</em>. Information about the user that created the topic. Currently, it is always present |
...
| can_manage_direct_messages | Boolean | false | <em>Optional</em>. <em>True</em>, if the administrator can manage direct messages of the channel and decline suggested posts; for channels only |
...
#### ChatMemberAdministrator
    ...
    ChatMemberAdministrator(status: String, user: User, can_be_edited: Boolean, is_anonymous: Boolean, can_manage_chat: Boolean, can_delete_messages: Boolean, can_manage_video_chats: Boolean, can_restrict_members: Boolean, can_promote_members: Boolean, can_change_info: Boolean, can_invite_users: Boolean, can_post_stories: Boolean, can_edit_stories: Boolean, can_delete_stories: Boolean, can_post_messages: Boolean, can_edit_messages: Boolean, can_pin_messages: Boolean, can_manage_topics: Boolean, can_manage_direct_messages: Boolean, custom_title: String)
...
| chat_id | IntegerOrString | true | Unique identifier for the target chat or username of the target supergroup (in the format <code>@supergroupusername</code>). Channel direct messages chats and channel chats aren't supported. |
...
#### forwardMessages
    ...
    forwardMessages(chat_id: IntegerOrString, message_thread_id: Integer, direct_messages_topic_id: Integer, from_chat_id: IntegerOrString, message_ids: List<Integer>, disable_notification: Boolean, protect_content: Boolean)
...
#### copyMessages
    ...
    copyMessages(chat_id: IntegerOrString, message_thread_id: Integer, direct_messages_topic_id: Integer, from_chat_id: IntegerOrString, message_ids: List<Integer>, disable_notification: Boolean, protect_content: Boolean, remove_caption: Boolean)
...
#### sendPhoto
    ...
    sendPhoto(business_connection_id: String, chat_id: IntegerOrString, message_thread_id: Integer, direct_messages_topic_id: Integer, photo: InputFileOrString, caption: String, parse_mode: ParseMode, caption_entities: List<MessageEntity>, show_caption_above_media: Boolean, has_spoiler: Boolean, disable_notification: Boolean, protect_content: Boolean, allow_paid_broadcast: Boolean, message_effect_id: String, suggested_post_parameters: SuggestedPostParameters, reply_parameters: ReplyParameters, reply_markup: KeyboardOption)
...
#### sendVideo
    ...
    sendVideo(business_connection_id: String, chat_id: IntegerOrString, message_thread_id: Integer, direct_messages_topic_id: Integer, video: InputFileOrString, duration: Integer, width: Integer, height: Integer, thumbnail: InputFileOrString, cover: InputFileOrString, start_timestamp: Integer, caption: String, parse_mode: ParseMode, caption_entities: List<MessageEntity>, show_caption_above_media: Boolean, has_spoiler: Boolean, supports_streaming: Boolean, disable_notification: Boolean, protect_content: Boolean, allow_paid_broadcast: Boolean, message_effect_id: String, suggested_post_parameters: SuggestedPostParameters, reply_parameters: ReplyParameters, reply_markup: KeyboardOption)
...
#### sendVoice
    ...
    sendVoice(business_connection_id: String, chat_id: IntegerOrString, message_thread_id: Integer, direct_messages_topic_id: Integer, voice: InputFileOrString, caption: String, parse_mode: ParseMode, caption_entities: List<MessageEntity>, duration: Integer, disable_notification: Boolean, protect_content: Boolean, allow_paid_broadcast: Boolean, message_effect_id: String, suggested_post_parameters: SuggestedPostParameters, reply_parameters: ReplyParameters, reply_markup: KeyboardOption)
...
#### sendPaidMedia
    ...
    sendPaidMedia(business_connection_id: String, chat_id: IntegerOrString, message_thread_id: Integer, direct_messages_topic_id: Integer, star_count: Integer, media: List<InputPaidMedia>, payload: String, caption: String, parse_mode: ParseMode, caption_entities: List<MessageEntity>, show_caption_above_media: Boolean, disable_notification: Boolean, protect_content: Boolean, allow_paid_broadcast: Boolean, suggested_post_parameters: SuggestedPostParameters, reply_parameters: ReplyParameters, reply_markup: KeyboardOption)
...
#### sendMediaGroup
    ...
    sendMediaGroup(business_connection_id: String, chat_id: IntegerOrString, message_thread_id: Integer, direct_messages_topic_id: Integer, media: List<InputMedia>, disable_notification: Boolean, protect_content: Boolean, allow_paid_broadcast: Boolean, message_effect_id: String, reply_parameters: ReplyParameters)
...
| can_manage_direct_messages | Boolean | false | Pass <em>True</em> if the administrator can manage direct messages within the channel and decline suggested posts; for channels only |
...
| name | type | required | description |
|---|---|---|---|
| chat_id | IntegerOrString | true | Unique identifier for the target chat or username of the target supergroup or channel (in the format <code>@channelusername</code>). Channel direct messages chats aren't supported; leave the corresponding channel instead. |
...
| name | type | required | description |
|---|---|---|---|
| chat_id | IntegerOrString | true | Unique identifier for the target chat or username of the target channel (in the format <code>@channelusername</code>). Channel direct messages chats can't be verified. |
| custom_description | String | false | Custom description for the verification; 0-70 characters. Must be empty if the organization isn't allowed to provide a custom verification description. |
...
#### approveSuggestedPost
    ...
    approveSuggestedPost(chat_id: Integer, message_id: Integer, send_date: Integer)
...
| name | type | required | description |
|---|---|---|---|
| chat_id | Integer | true | Unique identifier for the target direct messages chat |
| message_id | Integer | true | Identifier of a suggested post message to approve |
| send_date | Integer | false | Point in time (Unix timestamp) when the post is expected to be published; omit if the date has already been specified when the suggested post was created. If specified, then the date must be not more than 2678400 seconds (30 days) in the future |

#### declineSuggestedPost

    declineSuggestedPost(chat_id: Integer, message_id: Integer, comment: String)
...
|---|---|---|---|
| chat_id | Integer | true | Unique identifier for the target direct messages chat |
| message_id | Integer | true | Identifier of a suggested post message to decline |
| comment | String | false | Comment for the creator of the suggested post; 0-128 characters |
...
#### sendSticker
    ...
    sendSticker(business_connection_id: String, chat_id: IntegerOrString, message_thread_id: Integer, direct_messages_topic_id: Integer, sticker: InputFileOrString, emoji: String, disable_notification: Boolean, protect_content: Boolean, allow_paid_broadcast: Boolean, message_effect_id: String, suggested_post_parameters: SuggestedPostParameters, reply_parameters: ReplyParameters, reply_markup: KeyboardOption)
