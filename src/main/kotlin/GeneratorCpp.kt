...
private fun generateCppCMakeLists(files: List<CppModelFile>): String = buildString {
    ...
    #install(TARGETS tgbot-lib
    #    EXPORT tgbot-libTargets
    ...
    #)
    #install(EXPORT tgbot-libTargets
    #    FILE tgbot-libTargets.cmake
    #    NAMESPACE tgbot-lib::
    #    DESTINATION ${'$'}{CMAKE_INSTALL_LIBDIR}/cmake/tgbot-lib
    #)
    # Usage in other projects:
    #   add_subdirectory(path/to/telegram-api-generator/example/cpp)
    #   target_link_libraries(your_target PRIVATE tgbot-lib)
    """.trimIndent()
    )
}
...
private fun getDefaultValueForCppType(type: String, methodName: String?, propertyName: String? = null): String {
    ...
    return when (type) {
        ...
        "bool" -> " = false"
        "std::string" -> " = \"\""
        else -> type.startsWith("std::") ? " = $type()" : " = nullptr"
    }
}
    ...
    if (docParameters.isEmpty()) {
        ...
        val paramsSignature = docParameters.joinToString(", ") { parameter ->
            ...
            if (!parameter.required) {
                defaultValue = getDefaultValueForCppType(paramType, name, paramName)
            }
    ...
    if (docParameters.isEmpty()) {
        ...
        val paramsSignature = docParameters.joinToString(", ") { parameter ->
            ...
            val paramType = when (baseType) {
                ...
                else -> baseType.startsWith("std::") ? "const $baseType&" : "$baseType"
            }
