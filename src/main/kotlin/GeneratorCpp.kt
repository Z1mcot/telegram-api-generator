...
fun List<DocSection>.toCppModels() = buildString {
    ...
    if (sortedDocTypes.isNotEmpty()) {
        ...
        sortedDocTypes.forEach { docTypeWithSection ->
            ...
            appendLine()
            // appendLine(withIndent(docType.toCppSerialization(), "    "))
            // appendLine()
        }
    }
...
private fun generateVectorSerialization(
    ...
    // Handle both std::vector<ElementType> and std::optional<std::vector<ElementType>>
    val vectorType = if (fieldType.startsWith("std::optional<")) {
        fieldType.removePrefix("std::optional<").removeSuffix(">")
    } else {
        fieldType
    }
    
    // Extract element type from std::vector<ElementType>
    if (!vectorType.startsWith("std::vector<")) {
        // Not a vector type - return empty string
        return@buildString
    }
    
    val elementType = vectorType.removePrefix("std::vector<").removeSuffix(">")
    
    val vectorAccessor = if (required) fieldName else "${fieldName}.value()"
    val indent = if (required) "" else "    "
    
    if (!required) {
        appendLine("        if ($fieldName.has_value()) {")
    }
    
    // Check if element type is a basic JSON type
    if (BASIC_JSON_TYPES_IN_CPP.contains(elementType)) {
        // Direct assignment, no conversion needed
        appendLine("${indent}        j[\"$jsonFieldName\"] = $vectorAccessor;")
    } else if (elementType.startsWith("std::vector<")) {
        // Nested vector - recursively handle by generating nested loops
        append(generateNestedVectorSerialization(elementType, vectorAccessor, fieldName, jsonFieldName, indent))
    } else {
        // Complex type - call to_json on each element
        val tempVarName = "${fieldName}_values"
        appendLine("${indent}        std::vector<json> $tempVarName;")
        appendLine("${indent}        $tempVarName.reserve($vectorAccessor.size());")
        appendLine("${indent}        for (auto& e : $vectorAccessor) {")
        appendLine("${indent}            $tempVarName.push_back(e.to_json());")
        appendLine("${indent}        }")
        appendLine("${indent}        j[\"$jsonFieldName\"] = $tempVarName;")
    }
...
private fun generateNestedVectorSerialization(
    elementType: String,
    ...
    // Recursively extract nested vector types
    // elementType is already a vector type (std::vector<...>)
    fun extractVectorDepth(type: String): Pair<Int, String> {
        return if (type.startsWith("std::vector<")) {
            val innerType = type.removePrefix("std::vector<").removeSuffix(">")
            val (innerDepth, innermost) = extractVectorDepth(innerType)
            Pair(innerDepth + 1, innermost)
        } else {
            Pair(0, type)
        }
    }
    
    val (depth, innermostType) = extractVectorDepth(elementType)
    val topLevelVarName = "${baseFieldName}_values"
    
    // depth represents how many levels of vectors we have
    // depth 1 = std::vector<T> (single level)
    // depth 2 = std::vector<std::vector<T>> (nested)
    // Since elementType is already a vector, depth will be at least 1
    
    if (depth == 1) {
        // This shouldn't happen since we only call this for nested vectors
        // But handle it: vector of non-vector type
        appendLine("${baseIndent}        std::vector<json> $topLevelVarName;")
        appendLine("${baseIndent}        $topLevelVarName.reserve($vectorAccessor.size());")
        appendLine("${baseIndent}        for (auto& e : $vectorAccessor) {")
        if (BASIC_JSON_TYPES_IN_CPP.contains(innermostType)) {
            appendLine("${baseIndent}            $topLevelVarName.push_back(e);")
        } else {
            appendLine("${baseIndent}            $topLevelVarName.push_back(e.to_json());")
        }
        appendLine("${baseIndent}        }")
        appendLine("${baseIndent}        j[\"$jsonFieldName\"] = $topLevelVarName;")
    } else {
        // Nested vector (depth >= 2): std::vector<std::vector<...>>
        // Generate nested loops
        val innerVarName = "${baseFieldName}_inner"
        
        appendLine("${baseIndent}        std::vector<json> $topLevelVarName;")
        appendLine("${baseIndent}        $topLevelVarName.reserve($vectorAccessor.size());")
        appendLine("${baseIndent}        for (auto& outer : $vectorAccessor) {")
        appendLine("${baseIndent}            std::vector<json> $innerVarName;")
        appendLine("${baseIndent}            $innerVarName.reserve(outer.size());")
        appendLine("${baseIndent}            for (auto& e : outer) {")
        
        if (BASIC_JSON_TYPES_IN_CPP.contains(innermostType)) {
            appendLine("${baseIndent}                $innerVarName.push_back(e);")
        } else if (innermostType.startsWith("std::vector<")) {
            // Triple+ nested - for now, call to_json() which should handle it
            // In a more sophisticated implementation, we could generate more nested loops
            appendLine("${baseIndent}                $innerVarName.push_back(e.to_json());")
        } else {
            appendLine("${baseIndent}                $innerVarName.push_back(e.to_json());")
        }
        
        appendLine("${baseIndent}            }")
        appendLine("${baseIndent}            $topLevelVarName.push_back($innerVarName);")
        appendLine("${baseIndent}        }")
        appendLine("${baseIndent}        j[\"$jsonFieldName\"] = $topLevelVarName;")
    }
}
...
private fun DocType.toCppStruct() = buildString {
    ...
    if (docFields.isEmpty()) {
        ...
        docFields.forEachIndexed { index, field ->
            ...
            if (BASIC_JSON_TYPES_IN_CPP.contains(fieldType) || BASIC_TG_TYPES.contains(fieldType) || vectors.contains(fieldName)) {
                appendLine("    ${fieldType} $fieldName;")    
            } else if (field.required) {
                appendLine("    std::shared_ptr<${fieldType}> $fieldName;")
                ptrs.add(fieldName)
            } else {
                appendLine("    ${fieldType} $fieldName;")
            }
    ...
    if (docFields.isEmpty()) {
        ...
        docFields.forEach { field ->
            ...
            var toJsonPostfix = "";
            if (ptrs.contains(fieldName) && explicitlySerializedFields.contains(fieldName)) {
                toJsonPostfix = "->to_json()"
            } else if (explicitlySerializedFields.contains(fieldName)) {
                toJsonPostfix = ".to_json()"
            }

            if (vectors.contains(fieldName)) {
                append(generateVectorSerialization(fieldType, fieldName, jsonFieldName, field.required))
            } else if (field.required) {
                appendLine("        j[\"$jsonFieldName\"] = $fieldName$toJsonPostfix;")
            } else {
                if (ptrs.contains(fieldName)) {
                    appendLine("        if ($fieldName->has_value()) {")
                } else {
                    appendLine("        if ($fieldName.has_value()) {")
                }
                
                appendLine("            j[\"$jsonFieldName\"] = $fieldName.value()$toJsonPostfix;")
                appendLine("        }")
            }
        }
    }
...
private fun DocField.toCppFieldType(className: String?) =
    if (required) type.toCppTypeWithValueClasses(className, name) else "std::optional<${type.toCppTypeWithValueClasses(className, name)}>"
